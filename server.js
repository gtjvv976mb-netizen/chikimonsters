...
app.get("/health", async (_q, res) => res.json({
  ...
  rewardRatePerMin: RATE, maxClaimSol: CAP,
}));
...
app.post("/claim", async (req, res) => {
  ...
  const compute = (p) => {
    ...
    let amt = Math.min(minutes * RATE, CAP, Math.max(0, DAILY_CAP - daily), (WALLET_DAILY > 0 ? Math.max(0, WALLET_DAILY - walletPaid) : Infinity), Math.max(0, pool - RESERVE));
