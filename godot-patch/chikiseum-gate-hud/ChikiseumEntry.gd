...
func _build_entry() -> void:
    ...
    var subtitle:=Label.new();subtitle.name="EntryPurpose";subtitle.text="ONE CHAMPION  ·  REAL-TIME DUELS"
    subtitle.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
    subtitle.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER;subtitle.add_theme_font_size_override("font_size",11)
    subtitle.add_theme_color_override("font_color",ENTRY_SKIN.CYAN)
    subtitle.add_theme_color_override("font_shadow_color",Color(0,0,0,.85))
    ...
    var identity_heading:=Label.new();identity_heading.text="YOUR CHAMPION";identity_heading.add_theme_font_size_override("font_size",10)
    identity_heading.add_theme_color_override("font_color",ENTRY_SKIN.GOLD);identity.add_child(identity_heading)
    ...
    var secondary:=HBoxContainer.new();secondary.name="EntrySecondaryActions";secondary.add_theme_constant_override("separation",6);column.add_child(secondary)
    ...
    _refresh_online.pressed.connect(_refresh_roster);secondary.add_child(_refresh_online)
    var cup:=Button.new();cup.name="OpenExistingCup";cup.text="Chikoria Cup";cup.focus_mode=Control.FOCUS_NONE
    _entry_button(cup);cup.pressed.connect(_open_cup);secondary.add_child(cup)
    var exit:=Button.new();exit.name="ReturnToChikoria";exit.text="Return";_entry_button(exit)
    exit.tooltip_text="Return to Chikoria";exit.pressed.connect(close);secondary.add_child(exit)
...
func _layout() -> void:
    ...
    _entry_banner.custom_minimum_size.y=54 if compact else 66
    _entry_column.add_theme_constant_override("separation",5 if compact else 7)
    _fighter_name.add_theme_font_size_override("font_size",15 if compact else 17)
    ...
    var vertical_margin:=21 if screen.y<400 else 25 if compact else 28
