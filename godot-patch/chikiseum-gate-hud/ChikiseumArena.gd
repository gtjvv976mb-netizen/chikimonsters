...
func _skin_lobby_field(field: Control) -> void:
	# The same quiet carved-stone inputs serve practice and verified online lobby.
	# This is strictly local presentation; option metadata and validation are unchanged.
	if field is OptionButton:
		UISkin.button(field as Button)
		return
	if not field is LineEdit: return
	var normal := StyleBoxFlat.new()
	normal.bg_color = Color("201d2b")
	normal.border_color = Color("766249")
	normal.set_border_width_all(1)
	normal.set_corner_radius_all(4)
	normal.content_margin_left = 9; normal.content_margin_right = 9
	normal.content_margin_top = 6; normal.content_margin_bottom = 6
	field.add_theme_stylebox_override("normal", normal)
	var focus := normal.duplicate() as StyleBoxFlat
	focus.border_color = UISkin.GOLD; focus.set_border_width_all(2)
	field.add_theme_stylebox_override("focus", focus)
	field.add_theme_color_override("font_color", UISkin.LIGHT)
	field.add_theme_color_override("font_placeholder_color", UISkin.MUTED)
...
func _build_ui() -> void:
	...
	var lobby_stone := UISkin.panel(UISkin.GOLD,true)
	lobby_stone.bg_color = Color("191823f7")
	lobby_stone.border_color = Color("b9975d")
	lobby_stone.border_width_top = 3
	lobby_stone.shadow_color = Color(0.02,0.01,0.04,0.58)
	lobby_stone.shadow_size = 18
	lobby_stone.content_margin_left = 18; lobby_stone.content_margin_right = 18
	lobby_stone.content_margin_top = 12; lobby_stone.content_margin_bottom = 15
	_lobby.add_theme_stylebox_override("panel",lobby_stone); add_child(_lobby)
	...
	var stack := VBoxContainer.new(); stack.size_flags_horizontal = Control.SIZE_EXPAND_FILL; stack.add_theme_constant_override("separation", 11); scroll.add_child(stack)
	_lobby_banner = TextureRect.new(); _lobby_banner.name = "OfficialLobbyBanner"
	_lobby_banner.texture = OFFICIAL_BANNER; _lobby_banner.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	_lobby_banner.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	_lobby_banner.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_lobby_banner.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_lobby_banner.custom_minimum_size = Vector2(0,72); stack.add_child(_lobby_banner)
	var crest_line := HBoxContainer.new(); crest_line.name = "ArenaCrestRule"
	crest_line.add_theme_constant_override("separation",6); stack.add_child(crest_line)
	for color in [UISkin.VIOLET,UISkin.GOLD,UISkin.CYAN]:
		var rule := ColorRect.new(); rule.color = color; rule.mouse_filter = Control.MOUSE_FILTER_IGNORE
		rule.custom_minimum_size = Vector2(0,2); rule.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		crest_line.add_child(rule)
	var lobby_title := _label("CHOOSE YOUR DUEL" if online_entry_mode else "PRACTICE HALL", 18, UISkin.LIGHT)
	lobby_title.add_theme_font_override("font", BOLD); stack.add_child(lobby_title)
	var introduction := _label("ONE CHIKIMON · REAL-TIME COMBAT · NO TURNS", 11, UISkin.MUTED)
	introduction.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; stack.add_child(introduction)
	...
	_skin_lobby_field(_level.get_line_edit())
	...
	_skin_lobby_field(_stake.get_line_edit())
	...
	if reference_image_mode:
		# Preserve the title, concise rules and the two art-fixture pickers after
		# adding the official banner and crest rule above them.
		for index in range(5, stack.get_child_count() - 2): (stack.get_child(index) as Control).hide()
...
func _layout() -> void:
	...
	if _lobby_banner != null:
		_lobby_banner.custom_minimum_size.y = 42 if mobile and not portrait else 56 if mobile else 72
