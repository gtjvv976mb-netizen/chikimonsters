...
func _build() -> void:
	...
	if _panel == null:
		...
		add_child(_panel)
		_panel.resized.connect(_layout_gate_frame)
	if _gate_frame != null:
		_gate_frame.visible = false
...
func _layout_gate_frame() -> void:
	if _gate_frame == null or _panel == null or not _gate_frame.visible or _phase != "intro":
		return
	# The compact control card sits entirely inside the AI-authored arch. Scaling that full arch
	# INTO the card made its top jewel cover the title and its lower torchwork cover reward odds.
	# These outer margins put every ornamental pixel outside the readable control region.
	var breathing := Vector2(float(_sp(120.0)), float(_sp(115.0)))
	_gate_frame.position = _panel.position - breathing
	_gate_frame.size = _panel.size + breathing * 2.0
