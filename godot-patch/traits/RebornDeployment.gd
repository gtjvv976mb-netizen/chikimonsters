...
const INK := Color("f1e6d2")
const MUTED := Color("ad9bb6")
const VIOLET := Color("bf88f3")
const GOLD := Color("cda966")
...
func setup(units: Array, selected_uid: String, sandbox: bool, can_enter: bool,
	...
	var title := _label("WICKED TEMPLE", 19 if compact else 23, INK, true)
	...
	seam.color = Color("675032")
	seam.custom_minimum_size.y = _px(1)
	...
	var go := _button("ENTER THE TEMPLE", true)
	go.name = "WagerEnter"
...
func _hero(unit: Dictionary) -> void:
	...
	panel.add_theme_stylebox_override("panel", _plate(Color("21172c"), Color("392940"), 7 if _compact else 9))
	add_child(panel)
...
func _plate(fill: Color, edge: Color, padding: int) -> StyleBoxFlat:
	...
	style.set_corner_radius_all(_px(5))
...
func _style_button(button: Button, primary: bool) -> void:
	...
	button.add_theme_font_size_override("font_size", _px(11))
	for state in ["font_color", "font_hover_color", "font_pressed_color", "font_focus_color"]:
		button.add_theme_color_override(state, INK)
	button.add_theme_color_override("font_disabled_color", MUTED)
	button.add_theme_stylebox_override("normal", _plate(Color("653695") if primary else Color("241a30"),
		GOLD if primary else Color("4c385d"), 8))
	button.add_theme_stylebox_override("hover", _plate(Color("7a44b0") if primary else Color("382647"), VIOLET, 8))
	button.add_theme_stylebox_override("pressed", _plate(Color("492568") if primary else Color("1a1222"), GOLD, 8))
	button.add_theme_stylebox_override("disabled", _plate(Color("2c2434"), Color("4b4155"), 8))
