...
func _check_screen(phase: String) -> void:
	...
	if phase == "intro_initial":
		var gate := _temple._gate_frame
		if _mobile:
			_ok(gate != null and not gate.visible,
				"mobile entry uses the legible compact gate instead of squeezing the ornate frame")
		else:
			_ok(gate != null and gate.visible and gate.material is ShaderMaterial,
				"desktop gate displays its art with the exterior canvas mask")
			if gate != null and gate.visible:
				var frame := _bounds(gate)
				var opening := Rect2(frame.position + frame.size * Vector2(0.095, 0.140),
					frame.size * Vector2(0.810, 0.725))
				_ok(_safe.grow(1.0).encloses(frame) and opening.encloses(panel),
					"desktop gate and every control stay within the viewport and inner arch")
	var buttons_ok := true
	var count := 0
