...
> **Not compiled.** These sources were written without a Mac. They are structurally checked —
> braces balance, no duplicate types, every `model.…` member Screens.swift touches exists on
> ShellModel, the privacy manifest and Info.plist parse, and the project's object graph closes —
> and three compile errors were caught by review before they shipped, the last being
> `isInspectable` used without an availability check. But "it builds" is a claim only Xcode can
> make. Expect to fix something the first time you press Run.
...
1. **The `/link/*` backend routes do not exist yet.** Without `/link/redeem` the pairing screen
   has nothing to call, so **the app cannot sign in at all**. This is the one true blocker.
2. **The iOS pack must be published.** `realm/index.pck.ios.lite.*` is in this repo and the
   loader prefers it whenever `CHIK_IOS_APP` is set — but it only takes effect once `realm/` is
   published. Until then the app boots on the website's pack, with the Trading Post, the
   *Connect Phantom Wallet* button and the chat box all visible.

Two things this section used to list are no longer true, and both were settled by reading the
recovered game: PvP needs **no** new export (the Chikiseum already refuses anything that is not
`currency: "NONE"`), and there are no `season_*` routes to build.
...
There is one. This section used to describe building it by hand; you do not have to.

```sh
open ios/Chikoria.xcodeproj
```

It is generated by [`make-xcodeproj.py`](make-xcodeproj.py) and committed, with a shared scheme so
`xcodebuild` works too. Everything is already wired: the four Swift files compile, the asset
catalog and `PrivacyInfo.xcprivacy` are bundled, `Info.plist` is the explicit one in
`Chikoria/`, and the bundle id is `com.chikimonsters.Chikoria`.

**Adding a file later:** drop it in `ios/Chikoria/` and re-run `python3 ios/make-xcodeproj.py`.
Sources are discovered, not listed, and the object ids are derived from names, so regenerating
produces a readable diff rather than a churn of random hex.

**Checking it without opening Xcode:**

```sh
python3 ios/make-xcodeproj.py --check
```

That parses the project back and asserts every referenced object id is defined, every source is in
the Sources phase, every resource is in Resources, and the `Info.plist` carries the keys a bundle
needs. It was written because this project was generated without a Mac to test on — it does not
prove Xcode is happy, but it catches the class of error that otherwise costs you an afternoon.

**On the icon:** `Assets.xcassets/AppIcon.appiconset` holds a 1024×1024 icon cropped from
`realm/loading.png` — on-brand and valid (square, opaque, no alpha, which is what App Review
requires). It is a scene, not a mark, so it will read as a blur of colour at 60×60. Replace it
with a purpose-designed icon before you ship if you can; the build does not depend on it.
...
- **Minimum Deployments**: **iOS 16.0** — already set by the project. The sources use nothing
  newer: the only API above it is `WKWebView.isInspectable` (16.4), which is behind an
  `if #available` check. Raising it to 17 gains nothing and drops devices.
